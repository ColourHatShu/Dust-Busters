-- 0022_booking_notes.sql
-- Customer special instructions / scope for a booking ("focus on the kitchen",
-- "pets inside", access notes). Adds bookings.notes and a 6th request_booking
-- param p_notes (DEFAULT NULL) so existing callers are unchanged. Recreates the
-- 0021 body + the notes column on insert. (Shown to the cleaner only after the
-- deposit is paid — same gate as the address.)

ALTER TABLE bookings ADD COLUMN IF NOT EXISTS notes text;

DROP FUNCTION IF EXISTS request_booking(timestamptz, numeric, text, text, uuid);
...
begin
  ...
  insert into bookings (customer_id, scheduled_at, hours, area,
                        ...
                        platform_fee, cleaner_payout, notes, status)
